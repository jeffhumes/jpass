...
- [~] Support multiple separate vaults, let users name and rename each vault, choose which vault to open at startup, mark one vault as the default, keep each vault profile and encrypted data separate, and allow switching between vaults through an application menu option; non-desktop platform adapters remain pending.
