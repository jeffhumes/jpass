...
- [~] Support multiple separate vaults, let users name and rename each vault, choose which vault to open at startup, mark one vault as the default, delete the currently open vault only after a validated backup and confirmation, keep each vault profile and encrypted data separate, and allow switching between vaults through an application menu option; non-desktop platform adapters remain pending.
...
- [~] Support local vault export and backup. Encrypted desktop backups include the vault name and identity, and deleted vault backups can be restored as separate vault profiles; native file destinations and additional export formats remain pending.
